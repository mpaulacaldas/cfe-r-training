...
chile_head <- read_csv(
  url_chile,
  n_max = 5 # replace the ... with a number between 5 and 10
  )
...
germany_sheet2 <- read_excel("deaths-deu.xlsx", sheet = 2)
...
excel_sheets("deaths-deu.xlsx")

# Read the BL_2016_2021_Monate_AG_Ins sheet on your own. How many lines would
# you skip?
deaths_deu_bl <- read_excel("deaths-deu.xlsx", sheet = "BL_2016_2021_Monate_AG_Ins")
...
deaths_japan <- read_csv("deaths-japan.csv", 
                         locale = locale(encoding = "SHIFT-JIS"))

# Save data ---------------------------------------------------------------

# Choose one of the datasets we saw today and save it to an RDS file
write_csv(deaths_japan, "deaths-japan_utf8.csv")
