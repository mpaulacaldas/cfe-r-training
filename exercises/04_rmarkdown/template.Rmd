...
```{r setup, echo = FALSE}
knitr::opts_chunk$set(echo = params$show, message = FALSE)
```
...
On average, labour productivity in `r params$country_selected` is `r scales::comma(average_gva * 1000)` USD per worker. The region with the highest labour productivity has a gross value added per worker of `r scales::comma(max_gva * 1000)` USD. Its GVA per worker is `r round(max_gva / min_gva, digits = 1)` times that of the region with the lowest GVA per worker at `r scales::comma(min_gva * 1000)` USD. 
