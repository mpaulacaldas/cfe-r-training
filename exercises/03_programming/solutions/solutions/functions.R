...
(x - min(x, na.rm = y)) / (max(x, na.rm = y) - min(x, na.rm = y))

rescale <- function(x, y = TRUE) {
  (x - min(x, na.rm = y)) / (max(x, na.rm = y) - min(x, na.rm = y))
}

testing_vector <- c(0, 98, 1000)

rescale(testing_vector)
...
# There is a "gotcha" in this exercise. As you will see, the two data frames are 
# not the same. This is because the original code had a copy-paste error. This 
# demonstrates one advantage of functions: they reduce the likelihood of human
# error.
out
out2
