...
data_raw <- read_csv(
  fs::path(dir_dataraw, "gva_per_worker.csv"),
  col_types = cols(
...
write_xlsx(
  data_final, 
  path = fs::path(dir_data, "worker_productivity.xlsx") 
)


# 3 advanced --------------------------------------------------------------

tl2 <- read_rds(fs::path(dir_dataraw, "tl2.rds"))
gva <- read_csv(
  fs::path(dir_dataraw, "gva_per_worker.csv"),
  col_types = cols(
    .default = col_character(),
...
gva %>%
  ...
  write_rds(fs::path(dir_data, "worker_productivity.rds"))
