---
title: "Good practice and workflow with R"
...
params:
  module: "5"
  url_slides:  "https://oecd-cfe-eds.github.io/cfe-r-training/05_goodpractice.html"
  url_project: "https://rstudio.cloud/project/3022018"
...
---
# `r emoji("warning")` Avoid using `setwd()`
...
knitr::include_graphics("https://oecd-cfe-eds.github.io/good-enough-practices/figures/project0.PNG")
```
]
...
# `r emoji("woman_technologist")` Create and use an RStudio project
...
`r emoji("warning")` When working with other Directorates, avoid using the `V:/` alias. The alias points to a different location for each Directorate.

---
class: exercise

# `r emoji("woman_technologist")` Leveraging the `config` R package
...
---
# `r emoji("floppy_disk")` Good names
...
.center[`r emoji("heavy_check_mark")` __Good__]
...
# `r emoji("woman_technologist")` Building a project's infrastructure
...
# `r emoji("woman_technologist")` Working interactively
...
# `r emoji("pencil")` Restyle code

Head over to the [RStudio Cloud Project](`r params$url_project`) and open the `air pollution.R` script. What would you change given what you have learned today?

```{r, echo = FALSE}
countdown::countdown(minutes = 5, top = 0, id = "style")
```
...
# `r emoji("woman_technologist")` Reviewing a script
...
`r emoji("warning")` Use the UNC path for `V:/` when you're working with colleagues from another Directorate! o
